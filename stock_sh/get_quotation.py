...
def getStockCode(stockCode = None):
    ...
    if stockCode == None:
        sql = '''
        select stock_code, stock_name from ext_data_stock.stock_base_info where stock_code like 'sh.6%';
        '''
    else:
        sql = '''
...
def main():
    ...
    print("开始获取{}的行情数据......".format(current_date))
    ##获取指数日K数据
    index_dict = {
                    'sh.000001':'上证综指',
                    'sh.000016':'上证50',
    ...
    index_data_rows = getIndexQuot(current_date, current_date, index_dict=index_dict)
    print("获取了{}条指数行情数据".format(index_data_rows))

    ## 获取个股日K数据
    stock_data_rows = getStockQuot(current_date, current_date, None)
...
if __name__ == "__main__":
    # 获取个股日K数据
    # res = getStockQuot('2006-01-01', '2020-12-31', 'sh.600074')
